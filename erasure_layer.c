...
fec_encoded_data* el_encode_blocks(int K,int N, const unsigned char *message,int message_length){
...
#ifdef DEBUG
    printf("\n\nErasure Layer: Original message: \n%s",message);
#endif
...
#ifdef DEBUG
    printf("\nErasure Layer: Allocating memory for information blocks");
#endif
    primary_blocks = malloc(sizeof(unsigned char*)*K);
    for(i=0; i<K; i++){
       primary_blocks[i] = calloc(block_size+1,sizeof(unsigned char));
       if(primary_blocks[i]==NULL){
#ifdef DEBUG
          printf("\n\nErasure Layer: Not enough memory.\n");
#endif
          return result;
       }
    }
...
#ifdef DEBUG
    printf("\nErasure Layer: Allocating memory for redundancy blocks");
#endif
    ...
    for(i=0;i<(N-K); i++){
       secondary_blocks[i] = calloc(block_size+1,sizeof(unsigned char));
...
#ifdef DEBUG
    printf("\n\nErasure Layer: Creating fec decoder with K = %d and N = %d",K,N);
#endif
...
#ifdef DEBUG
    printf("\n\nErasure Layer: Encoding message....");
#endif
...
#ifdef DEBUG
    printf("\n\nErasure Layer: Information blocks:");
    for(i=0; i<K; i++)
       printf("\n\nErasure Layer: Information block %d\n%s",i,primary_blocks[i]);
...
#ifdef DEBUG
    printf("\n\nErasure Layer: Redundancy blocks:");
#endif
...
#ifdef DEBUG
       printf("\n\nErasure Layer: Redundancy block %d\n%s",i,secondary_blocks[i]);
#endif
    }
...
int el_decode_blocks(fec_received_data *data,unsigned char *message){
...
#ifdef DEBUG
    printf("\n\nErasure Layer: Recontructing original blocks...");
    for(i=0; i<data->K; i++)
       if(data->index_blocks[i] >= data->K )
          printf("\n\nErasure Layer: Input block-> %d Redundancy Block\n%s",i,data->input_blocks[i]);
       else
          printf("\n\nErasure Layer: Input block-> %d Information Block\n%s",i,data->input_blocks[i]);
#endif
    ...
    for(i=0; i<data->K; i++){
       //data->output_blocks[i] = malloc(sizeof(unsigned char)*data->block_size+1);
       data->output_blocks[i] = calloc(data->block_size+1,sizeof(unsigned char));
...
#ifdef DEBUG
    printf("\n\nErasure Layer: Reconstructed message: \n%s",message);
#endif
